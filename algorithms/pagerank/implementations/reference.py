...
This is the oracle every engine is compared against, and a teaching artifact
...
# Gigi's canonical defaults for anything the engines do not agree on.
DEFAULT_DAMPING = 0.85
DEFAULT_TOLERANCE = 1e-12  # tighter than any engine, so the oracle is not the noise
...
def pagerank(nodes, edges, damping, tolerance, max_iterations, weighted, directed=True):
    ...
    the places engines quietly differ.
    """
    n = len(nodes)
    if n == 0:
        return {}, 0
...
def run(graph, params):
    ...
    # weight_property: None means "engine default", and this engine's default is
