# PageRank — engine notes

What follows was measured by `gigi verify pagerank`, not read from
documentation. Every claim here has a corresponding entry in `algorithm.yaml`
...
That is a 49% relative difference on node `a` between two engines running "the
...
parameter is deliberately tri-state (`null` = engine default, `false` =
explicitly unweighted, a string = use that column) so that the ambiguity is
visible in the call rather than hidden in the engine.
...
- **Dangling nodes** (`dangling-small`): all four engines redistribute the
  dangling mass uniformly. Agreement to `1e-13` against the reference for
  igraph, `1e-6` for the power-iteration engines.
...
equality: two engines can order tied-ish nodes differently at the sixth decimal
place. `comparison.absolute_tolerance` in `algorithm.yaml` is set to `1e-6` for
exactly this reason, and verification pins `tolerance: 1e-12` so that the
engines are compared at convergence rather than at their own stopping points.
