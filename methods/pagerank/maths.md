...
backends have all made the same choice.
...
it towards uniform. Not every backend implements this case — see
`rustworkx-directed-only` in `method.yaml`.
...
`10^-8`, which is why backends that cap at 100 iterations essentially never hit
...
around `1e-13` while power-iteration backends stop at their tolerance, typically
`1e-6`.
