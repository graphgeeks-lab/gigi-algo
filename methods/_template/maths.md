...
a choice open, because those are where backends diverge — and naming them here
...
- **Direction.** On a directed graph, "degree" can mean in-degree, out-degree
  ...
  so does `nx.degree_centrality`. A backend that returns in-degree only would
...
- **Parallel edges.** Whether two edges between the same pair count once or
  twice depends on whether the backend models a multigraph. Gigi preserves
  multiplicity ([ADR 0003](../../docs/adr/0003-graph-data-contract.md)).
- **Normalisation.** Some backends always normalise and give you no way to turn
