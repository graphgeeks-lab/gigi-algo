...
def degree_centrality(nodes, edges, normalized=True, directed=True):
    ...
    readings of "degree centrality", and backends do not all agree. Choices like
    this one are exactly what belongs in `maths.md` and, when backends differ
...
def run(graph, params):
    ...
    graph  -- a ConvertedGraph. For the reference backend, `graph.native` is
              {"nodes": [...], "edges": [(source, target, weight), ...]}.
    params -- canonical parameters, as named in method.yaml.
    ...
    value the backend chose for you. That record is the whole point: a backend
    default nobody wrote down is a backend default nobody can audit.
    """
