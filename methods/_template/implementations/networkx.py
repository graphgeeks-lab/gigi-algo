"""Backend implementation — a working example.

This file calls the backend. It never reimplements it: if you find yourself
writing a loop over nodes here, the code belongs in reference.py instead.

Conversion from Gigi's neutral GraphData to the backend's graph object has
already happened in gigi/backends/networkx.py, so `graph.native` is an
...
# Write the backend's own defaults down as constants. They are the values most
# likely to differ from another backend's, and the ones least likely to be
...
def run(graph, params):
    ...
    if not requested_normalized:
        # The backend gives us no way to turn normalisation off, so undo it and
