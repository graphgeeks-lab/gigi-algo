...
def review(method_id: str) -> Review:
    """Assemble the review for one algorithm."""
    spec = registry.load_method(method_id)
    result = Review(method_id=method_id, maturity=spec.maturity.value)
    ...
    cases = knownanswers.load_cases(method_id)
    failed_cases = [
        r for r in (knownanswers.run_case(method_id, c) for c in cases) if not r.passed
    ]
    result.checks.append(
        Check(
    ...
    implemented = set(registry.implemented_backends(method_id))
    for backend in sorted(set(registry.plausible_backends(spec)) - implemented):
        result.gaps.append(f"no {backend} implementation yet")
    # Only fixtures this method could actually consume. Telling a vector
    # measure it has not been run against a graph is noise, and noise in a
    # gap list is how gap lists stop being read.
    kinds = {str(i.kind) for i in spec.inputs}
    candidates = {d for kind in kinds for d in list_datasets(kind)}
    uncovered = sorted(candidates - set(spec.datasets))
    if uncovered:
    ...
    result.by_eye = _by_eye(spec, method_id)
    return result


def _by_eye(spec, method_id: str) -> list[ByEye]:
    ...
    if spec.provenance.original_authors or spec.provenance.precursors:
        items.append(
            ByEye(
                ...
                where="`provenance` in method.yaml",
