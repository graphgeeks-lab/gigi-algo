...
# The smallest and most awkward inputs of each kind: the ones where a
# definition stops deciding and an implementation has to. Keyed by input kind,
# because "the degenerate case" is not the same shape for a graph method and a
# pairwise measure -- a one-node graph still has a node, a one-vector fixture
# has no pairs at all.
DEGENERATE_FIXTURES: dict[str, tuple[str, ...]] = {
    "graph": ("empty", "single-node"),
    "vectors": ("vectors-single", "vectors-with-zero"),
}
...
class Requirement:
    ...
    check: Callable[[MethodSpec], "str | None"]  # None when met, else why not
...
def _known_answers(minimum: int) -> Callable[[MethodSpec], "str | None"]:
    def _check(spec: MethodSpec) -> str | None:
...
def _known_answers_derived(spec: MethodSpec) -> str | None:
    from gigi.knownanswers import load_cases
...
def _problems_named(spec: MethodSpec) -> str | None:
    if not spec.problems:
        return "solves no named problem, so nothing can recommend it"
    unknown = [p for p in [*spec.problems, *spec.intent.not_for] if not registry.problem_exists(p)]
    return f"unknown problem id(s): {', '.join(unknown)}" if unknown else None


def _provenance_cited(spec: MethodSpec) -> str | None:
...
def _divergences_testable(spec: MethodSpec) -> str | None:
...
def _divergences_have_choice_points(spec: MethodSpec) -> str | None:
...
def _divergences_credited(spec: MethodSpec) -> str | None:
...
def _degenerate_fixtures(spec: MethodSpec) -> str | None:
    wanted = [d for i in spec.inputs for d in DEGENERATE_FIXTURES.get(i.kind, ())]
    missing = [d for d in wanted if d not in spec.datasets]
    if missing:
        return f"not run against {', '.join(missing)}"
    return None


def _several_engines(spec: MethodSpec) -> str | None:
    backends = [e for e in registry.implemented_backends(spec.id) if e != "reference"]
    if len(backends) >= 2:
        return None
    return f"{len(backends)} non-reference backend(s); cross-backend evidence needs two"
...
REQUIREMENTS: list[Requirement] = [
    ...
    Requirement("problems_named", "it names the problem it solves", Maturity.emerging, _problems_named),
    ...
    Requirement("degenerate_fixtures", "runs against the degenerate fixtures for its input kind", Maturity.stable, _degenerate_fixtures),
    Requirement("several_engines", "two or more backends besides the reference", Maturity.stable, _several_engines),
]


def check(spec: MethodSpec) -> list[Outcome]:
...
def unmet(spec: MethodSpec) -> list[Outcome]:
...
def next_tier(spec: MethodSpec) -> tuple[Maturity | None, list[Outcome]]:
